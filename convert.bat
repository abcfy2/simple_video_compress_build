...
if /I "%~1"=="-n" (set OVERRIDE_OPTS=-n& shift & goto :parse_args)
if /I "%~1"=="--format" (set OUTPUT_FORMAT=%~2& shift & shift & goto :parse_args)
if /I "%~1"=="--h265" (set ENABLE_h265=1& shift & goto :parse_args)
if /I "%~1"=="--opus" (set ENABLE_OPUS=1& shift & goto :parse_args)
if /I "%~1"=="--hwencoder" (set HWENCODER=%~2& shift & shift & goto :parse_args)
if /I "%~1"=="--sub" (set ENABLE_SUB=1& shift & goto :parse_args)
if /I "%~1"=="--subenc" (set SUBENC=%~2& shift & shift & goto :parse_args)
if /I "%~1"=="--subsuffix" (set SUBSUFFIX=%~2& shift & shift & goto :parse_args)
if /I "%~1"=="--subdir" (set SUBDIR=%~2& shift & shift & goto :parse_args)
if /I "%~1"=="--scale" (set SCALE=%~2& shift & shift & goto :parse_args)
if /I "%~1"=="--framerate" (set FRAMERATE=%~2& shift & shift & goto :parse_args)
if /I "%~1"=="--videocopy" (set VIDEOCOPY=1& shift & goto :parse_args)
if /I "%~1"=="--audiocopy" (set AUDIOCOPY=1& shift & goto :parse_args)
if /I "%~1"=="--samplerate" (set SAMPLE_RATE=%~2& shift & shift & goto :parse_args)
if /I "%~1"=="--opts" (set FFMPEG_OPTS=%~2& shift & shift & goto :parse_args)
if /I "%~1"=="-j" (set join_mode=1& shift & goto :parse_args)
if /I "%~1"=="--join" (set join_mode=1& shift & goto :parse_args)
if /I "%~1"=="-o" (set OUT=%~2& shift & shift & goto :parse_args)
...
if /I "%~1"=="--loglevel" (set FFMPEG_PRE_OPTS=%FFMPEG_PRE_OPTS% -loglevel %~2& shift & shift & goto :parse_args)
...
:process_args_done
:: Detect codecs and build options
call :detect_and_build_opts
...
:: Check for H265 and build video options
if "%ENABLE_h265%"=="1" (
...
if defined DIR (set outdir=%DIR%) else (for %%F in ("%infile%") do set outdir=%%~dpF)
if not exist "%outdir%" mkdir "%outdir%"
if defined OUT (set outfile=%OUT%) else (set outfile=%fname%_enc.%OUTPUT_FORMAT%)

:: Build scale option
set SCALE_OPTS=
if defined SCALE set SCALE_OPTS=-vf scale=%SCALE%

:: Build frame rate option
set FRAMERATE_OPTS=
if defined FRAMERATE set FRAMERATE_OPTS=-r %FRAMERATE%

echo [CMD] %FFMPEG% -nostdin %OVERRIDE_OPTS% %FFMPEG_PRE_OPTS% -i "%fpath%" %VIDEO_OPTS% %SCALE_OPTS% %FRAMERATE_OPTS% %AUDIO_OPTS% %FFMPEG_OPTS% "%outdir%%outfile%"
echo.

%FFMPEG% -nostdin %OVERRIDE_OPTS% %FFMPEG_PRE_OPTS% -i "%fpath%" %VIDEO_OPTS% %SCALE_OPTS% %FRAMERATE_OPTS% %AUDIO_OPTS% %FFMPEG_OPTS% "%outdir%%outfile%"
...
:: Build options for join
set SCALE_OPTS=
if defined SCALE set SCALE_OPTS=-vf scale=%SCALE%
set FRAMERATE_OPTS=
if defined FRAMERATE set FRAMERATE_OPTS=-r %FRAMERATE%

%FFMPEG% -nostdin %OVERRIDE_OPTS% %FFMPEG_PRE_OPTS% -protocol_whitelist file,pipe -f concat -safe 0 -i "%concat_file%" %VIDEO_OPTS% %SCALE_OPTS% %FRAMERATE_OPTS% %AUDIO_OPTS% %FFMPEG_OPTS% "%DIR%\%OUT%"
